...
#define SERVO1_STEP 8 //Step Value for L/R Trigger [Base Joint]
#define SERVO4_STEP 8 //Step Value for L/R D-pad   [Claw]

//Define rate for servos controller by sticks (larger is faster)
#define SERVO2_RATE 4/3200
#define SERVO3_RATE 4/3200
  ...
  if(Xbox.XboxReceiverConnected || Xbox.Xbox360Connected) {
    for(uint8_t i=0; i<4;i++){
      if(Xbox.Xbox360Connected[i]){ //This loop executes only when Xbox 360 controller is connected
        ...
        if(Xbox.getAnalogHat(i,LeftHatY) > STICK_CENTER + DEADZONE 
          || Xbox.getAnalogHat(i,LeftHatY) < STICK_CENTER+ DEADZONE){
            s2 = s2 + (Xbox.getAnalogHat(i,LeftHatY) - STICK_CENTER)*SERVO2_RATE*SERVO2_DIRECTION;
          }
          if(s2 > SERVO2_MAX) s2 = SERVO2_MAX;
          if(s2 < SERVO2_MIN) s2 = SERVO2_MIN;
        
        if(Xbox.getAnalogHat(i,RightHatY) > STICK_CENTER + DEADZONE
          || Xbox.getAnalogHat(i,RightHatY) < STICK_CENTER + DEADZONE){
            s3 = s3 + (Xbox.getAnalogHat(i,RightHatY) - STICK_CENTER)*SERVO3_RATE*SERVO3_DIRECTION;
          }
        ...
        else if(Xbox.getButtonClick(i,RIGHT)){
          s4 = s4 - SERVO4_STEP*SERVO4_DIRECTION;
        }
